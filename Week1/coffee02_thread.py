...
def main():
    # คิวลูกค้า
    queue = ['A', 'B', 'C']
    print(f"{ctime()} === เริ่มระบบจำลองร้านกาแฟแบบ Multi-threading ===")
    start_time = time()
    ...
    for customer in queue:
        # สร้าง Thread ใหม่ โดยระบุไปวิ่งที่ฟังก์ชัน make_coffee และส่งชื่อลูกค้าไปเป็น argument
        t = threading.Thread(target=make_coffee, args=(customer,))
        threads.append(t)
        t.start() # สั่งให้ Thread เริ่มทำงานทันที (ไม่ต้องรอให้รอบบัดตัวไปเสร็จ)
